...
import { mergeLockFiles } from './mergeLockFiles';

export default class WorkspaceWebpackPlugin extends ResolvePlugin {
  constructor(private options: { workspaces: string[] }) {
    super();
  }
  ...
  apply(resolver: Resolver) {
    resolver.getHook(`before-module`).tapAsync(`WorkspaceWebpackPlugin`, (requestContext, resolveContext, callback) => {
      ...
      //let issuer = requestContext.issuer;
      
      console.log(request);

      let newYarnLock = mergeLockFiles(this.options.workspaces[0], this.options.workspaces[1]);
